...
def sortino_annualized(returns, freq=252, mar=0):
    neg_returns = returns[returns < mar]
    downside_std = neg_returns.std() * np.sqrt(freq)
    ann_return = returns.mean() * freq
    return ann_return / downside_std if downside_std != 0 else 0

# =============================
# AUTOARIMA FORECAST (MONTHLY)
# =============================
def autoarima_forecast_monthly(price_series, start_date, horizon_months, last_known_price):
    # Monthly resample (last day of month)
    monthly = price_series.resample("M").last()

    # Returns
    returns = make_returns(monthly)

    # Train ARIMA on returns
    model = pm.auto_arima(returns, seasonal=False, suppress_warnings=True, stepwise=True)

    # Future months
    future_end = pd.to_datetime(start_date) + pd.DateOffset(months=horizon_months-1)
    future_dates = pd.date_range(start_date, future_end, freq="M")

    # Forecast returns
    ret_fc = model.predict(n_periods=len(future_dates))
...
# =============================
# ROLLING AUTOARIMA FORECAST (MONTHLY)
# =============================
def rolling_autoarima_forecast(price_series):
    # Monthly resample
    monthly = price_series.resample("M").last()
    returns = make_returns(monthly)
    preds = []
    ...
    last_price = monthly.loc[:TRAIN_END].iloc[-1]
    ...
    for d in retrain_dates:
        ...
        model = pm.auto_arima(train_ret, seasonal=False, suppress_warnings=True, stepwise=True)

        future_dates = pd.date_range(d, min(d + pd.offsets.MonthEnd(1), pd.to_datetime(TEST_END)), freq="M")
        ret_fc = model.predict(n_periods=len(future_dates))
...
def plot_ef_separate(hist, forecast, realized, min_w=0.01, max_w=0.4):
    ...
    for ax, data, title in zip(
        ...
        ["Historical (Train)", "Forecast (ARIMA)", "Realized"]
    ):
...
st.title("📈 AI Portfolio Optimizer – AutoARIMA (Monthly)")
...
# =============================
df_close_daily = download_data(tickers, TRAIN_START, TRAIN_END)
df_close = df_close_daily.resample("M").last()
...
# =============================
# REALIZED DATA (MONTHLY)
# =============================
realized_close_daily = download_data(tickers, TEST_START, TEST_END)
realized_close = realized_close_daily.resample("M").last()
...
for t in tickers:
    rolling_forecast_df[t] = rolling_autoarima_forecast(df_close_daily[t])
...
for t in tickers:
    forecast_df[t] = autoarima_forecast_monthly(
        df_close_daily[t],
...
for t in tickers:
    ...
    ax.plot(rolling_forecast_df[t], label="ARIMA Rolling (2024-2025)", linestyle="--")
    ax.plot(forecast_df[t], label="ARIMA 2026 Forecast", linestyle=":")
    ax.set_title(t)
    ax.legend()
    ax.grid(alpha=0.3)
    st.pyplot(fig)
...
st.dataframe(metrics_df.style.background_gradient(cmap="RdYlGn", subset=["Sharpe Ratio"]))

# =============================
# EFFICIENT FRONTIER
# =============================
st.subheader("📊 Efficient Frontier")
plot_ef_separate(df_close, rolling_forecast_df, realized_close)
